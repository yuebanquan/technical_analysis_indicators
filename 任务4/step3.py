...
for sampleYear in range(1, 9):
    # 该样本内年份开始运行时间
    startTime = time.time()

    print("样本内年份为：" + str(sampleYear))
    bestMADf, backtestTradingDf, revenues = ts.slidingWindowTrading(df, backtestStartDate, backtestEndDate, sampleYear,
                                                                    lowShortEMA, highShortEMA, lowMediumEMA, highMediumEMA,
                                                                    lowLongEMA, highLongEMA, initBalance)
