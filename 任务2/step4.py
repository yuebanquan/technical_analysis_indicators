...
result4_1 = pd.DataFrame(columns=['样本内数据年份数', '第iyear窗口', '最佳短期均线','最佳中期均线', '样本内净资产'])
...
for iyear in range(0, sampleYear):
    ...
    sampleMAAndNetAssetDf = ts.getMAAndNetAsset(df, sampleStartDate, sampleEndDate, lowShortMA, highShortMA, lowMediumMA, highMediumMA)
    ...
    result4_1.loc[iyear] = [sampleYear, iyear, bestShortMA, bestMediumMA, sampleMostNetAsset]
...
backtestTradingDf.to_csv(result4_2File, encoding='utf-8-sig')
