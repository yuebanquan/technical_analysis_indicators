...
def getTestData(stockDataDf, testStartDate, testEndDate, forecastDays, pastDays):
    ...
    """
    testX = []  # 测试集X
    testY = stockDataDf.loc[testStartDate:testEndDate, 'Close']  # 测试集Y
...
def getRatio(realPreDf):
    realPreDfCopy = realPreDf.copy()
    realPreDfCopy.dropna(inplace=True)
    ratio = (np.abs(realPreDfCopy.real.values - realPreDfCopy.predict.values) / realPreDfCopy.real.values).mean()
    return ratio
...
def LSTMPredict(df, backtestStartDate, backtestEndDate, forecastDays, pastDays, maValue, stepMonth, sampleDataSize):
    ...
    # 遍历每个滑动窗口
    for sw in swZip:
        print(f"{str(sw[2])[:10]} to {str(sw[3])[:10]}")  # 打印进度
        ...
        testX, testY = getTestData(scaStockDataDf, testStartDate, testEndDate, forecastDays, pastDays)
    ...
    realY = inverse_transform_col(scaler, realY, 6)  # 将归一化后的结果逆转
