...
def getTestData(stockDataDf, testStartDate, testEndDate, forecastDays, pastDays):
    ...
    """
    testX = []  # 测试集X
    testY = stockDataDf.loc[testStartDate:testEndDate, 'label']  # 测试集Y
...
def LSTMPredict(df, backtestStartDate, backtestEndDate, forecastDays, pastDays, maValue, stepMonth, sampleDataSize):
    ...
    for sw in swZip:
        ...
        model = Sequential()
        # 构建第一层
        model.add(LSTM(10, input_shape=trainX.shape[1:], activation='relu',
        ...
        # pY1 = model.predict(testX).reshape(1, -1)
        # pY2 = model.predict(testX).reshape(1, -1).tolist()[0]
