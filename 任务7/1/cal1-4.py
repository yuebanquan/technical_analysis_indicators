...
newResultSrFile = r'./result/3_newResultSr.csv'
...
newResultSr = pd.Series([sampleYear, backtestNetAsset, backtestTotalRate, backtestCompoundRate, backtestMaxDrawDown,
                         backtestSharpeRatio],
                        index=['样本内数据年份数', '样本外净资产', '总收益率', '年复利', '最大回撤率', '夏普率'])

# 保存输出数据
newResultSr.to_csv(newResultSrFile, encoding='utf-8-sig', header=False)
