...
def strategy1(df, startDate, endDate, hold=0, showIndicator=False, showPreIndicator=False):
    # 交易信号
    buySign = 1  # 买入信号
    sellSign = -1  # 卖出信号
    ...
    # 取交易日期
    # 计算金叉 & 死叉、买入日期 & 卖出日期
    goldenCross = (dif > dea) & (dif <= dea).shift(1)
    deathCross = (dif < dea) & (dif >= dea).shift(1)
    ...
    # 判断第一个交易日是否买入
    if (dif[preStartDate] > dea[preStartDate]) and (hold == 0):
        strategyDf.loc[startDate, 'sign'] = buySign
    # 判断第一个交易日是否卖出
    elif (dif[preStartDate] < dea[preStartDate]) and (hold != 0):
...
def strategy2(df, startDate, endDate, DEA1, DEA2, hold=0, showIndicator=False, showPreIndicator=False):
    # 交易信号
    buySign = 1  # 买入信号
    sellSign = -1  # 卖出信号
    startOrEndSign = 0  # 交易上下限
...
def getAllStrategy(df, startDate, endDate, lowDEA1=-100, highDEA1=100, lowDEA2=-100, highDEA2=100):
    ...
    for DEA2 in range(lowDEA2, highDEA2 + 1):
        for DEA1 in range(lowDEA1, highDEA1 + 1):
            strategyDf = strategy2(df, startDate, endDate, DEA1, DEA2)
...
def slidingWindowTrading(df, backtestStartDate, backtestEndDate, sampleYear=8, lowDEA1=100, highDEA1=100, lowDEA2=-100,
    ...
    for iyear in range(0, backtestYear):
        ...
        # 将最佳MA放入滑动窗口进行回测
        slidingWindowStrategyDf = strategy2(df, slidingWindowStartDate, slidingWindowEndDate, bestDEA1, bestDEA2,
                                            hold=hold)
