...
def trading(strategyDf, initBalance=1000000, initHold=0):
    ...
    tradingDf['balance'] = nanList
    tradingDf['netAsset'] = nanList
    ...
    for i in range(0, len(tradingDf)):
        if signList[i] == buySign and hold == 0:
            ...
            if buynum != 0:
                ...
                partMaxNetAsset = netAssetList[i]
                tList[i] = partMaxNetAsset
        ...
        if (i != 0):
            # 更新
            if holdList[i] != holdList[i - 1]:
                partMaxNetAsset = netAssetList[i]
                tList[i] = partMaxNetAsset
            if holdList[i] != 0:
                if partMaxNetAsset < netAssetList[i]:
                    partMaxNetAsset = netAssetList[i]
                    tList[i] = partMaxNetAsset
                partDrawDown = (netAssetList[i] - partMaxNetAsset) / partMaxNetAsset

                if partDrawDown < -0.05:
