...
def slidingWindowTrading(df, backtestStartDate, backtestEndDate, sampleYear=8,
    ...
    bestDf = pd.DataFrame(columns=['样本内数据年份数', '第iyear窗口', 'bestMA', 'bestDEA1', 'bestDEA2', '样本内净资产'],
    ...
    for iyear in range(0, backtestYear):
        ...
        #
        # bestDf.loc[iyear, :] = [sampleYear, iyear, bestMA, bestDEA1, bestDEA2, bestNetAsset]

        bestDf = pd.read_csv(r"./result/2_1_最佳参数组合交易明细.csv")
        bestMA = bestDf.loc[iyear, 'bestMA']
