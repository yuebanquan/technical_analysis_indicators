...
def slidingWindowTrading(df, backtestStartDate, backtestEndDate, sampleYear=8, lowShortMA=1, highShortMA=8,
    ...
    bestMADf = pd.DataFrame(columns=['样本内数据年份数', '第iyear窗口', '最佳短期均线', '最佳中期均线', '最佳长期均线', '样本内净资产'])
    ...
    revenues = pd.Series([sampleYear, backtestNetAsset, backtestCompoundRate], index=['样本内数据年份数', '样本外净资产', '年均收益率复利'])
